...
listOne = ["Have a Great", "Have a Wonderful", "Happy"]
randOne = random.randint(0, 2)
greeting = listOne[randOne]

listTwo = ["All", "Everyone", "Everybody Out There"]
randTwo = random.randint(0, 2)
everyone = listTwo[randTwo]

listThree = ["Have a great day!", "Have a wonderful day!", "Have a fantastic day!"]
randThree = random.randint(0, 2)
...
if threeSidedCoinFlip == 2:
    ...
    symbol = str(pennySymbol)
    name = str(pennyName)
    exchange = str(pennyMarket)

name = (str(name)).rstrip("',)").lstrip("(u'")
symbol = (str(symbol)).rstrip("',)").lstrip("(u'")

if threeSidedCoinFlip == 2:
    exchange = (str(exchange)).rstrip("',)").lstrip("(u'")
